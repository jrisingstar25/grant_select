 ...
 * @since v.1.0.0
 */
function wppb_cpm_login_logout_title( $ID ) {
 ...
 * @since v.1.0.0
 */
function wppb_cpm_setup_nav_menu_item( $item ) {
	global $pagenow;
		...
		if( ! empty( $wppb_cpm_form_page_url ) ) {
			switch( $item->type ) {
				case 'wppb_cpm_login_logout' :
					...
					$item->title = wppb_cpm_login_logout_title( $item->ID );
					break;
...
add_filter( 'wp_setup_nav_menu_item', 'wppb_cpm_setup_nav_menu_item' );