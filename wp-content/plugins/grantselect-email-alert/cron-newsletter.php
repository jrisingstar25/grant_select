...
class GS_Cron_Newsletter{
    private $limit       = 'all';
    private $about       = '';
    private $subject     = 'Funding Opportunities from GrantSelect.com';
    private $report      = '';
    public $newsletter_ids = '';
    public $newsletter_test_ids = '';
    ...
    /**
     * Function get_newsletter_ids. Searching yesterday alerts
     * @params 
     * @return $res
     */
    function get_newsletter_ids( )
    {
        ...
        $sql = null;
        $sql_query = "SELECT  `id` FROM " . $wpdb->prefix . "gs_grants WHERE updated_at>'" . $date . " 00:00:00' and updated_at<'".date('Y-m-d')." 23:59:59' AND status='A' ORDER BY updated_at DESC";
        $sql = $wpdb->prepare( $sql_query, $keyword_str, $keyword_str );
        $data = $wpdb->get_results( $sql, "OBJECT_K" );
        $res = array_keys( $data );
        return implode(",", $res);
    }
    function get_newsletter_test_ids( )
    {
        global $wpdb;
        $res = array(); //initialize
        
        $sql = null;
        $sql_query = "SELECT  `id` FROM " . $wpdb->prefix . "gs_grants WHERE status='A' ORDER BY updated_at DESC limit 3";
        $sql = $wpdb->prepare( $sql_query, $keyword_str, $keyword_str );
        $data = $wpdb->get_results( $sql, "OBJECT_K" );
        $res = array_keys( $data );
        return implode(",", $res);
    ...
    public function send_test_email($data){
        $this->newsletter_test_ids = $this->get_newsletter_test_ids();
        if ($data){
            $count_of_subscr = 0;
            foreach ($data as $ea){
                ...
                $out_result = $this->search_for_testing();
                if (isset($out_result[0]) && $out_result[0] != '') {
                    ...
                    if (!empty($grant_info_out)){
                        $this->send_mail_to_subscr($grant_info_out, $ea);
                    }
    ...
    public function send_email($data){
        $this->newsletter_ids = $this->get_newsletter_ids();
        if ($data){
            $count_of_subscr = 0;
            foreach ($data as $ea){
                ...
                $out_result = $this->match_user_segment( $out_result, $ea->subscr_id );
                if (isset($out_result[0]) && $out_result[0] != '') {
                    ...
                    if ($count_alerts_for_subsc > 0) {
                        $this->report .= 'Subscriber: ' . '<a href=mailto:"' . $ea->email . '">' . $ea->email . '</a>  ' . $count_alerts_for_subsc . ' records Newsletter sent for <a href=mailto:"' . $ea->email . '">' . $ea->email . '</a> today <br />';
                    }
                    else {
    ...
    /**
     * Function to get the latest 3 grant to test newsletter
     * @return $res
     */
    function search_for_testing()
    {
        global $wpdb;
        $res = array(); //initialize
        ...
        $sql_query = "SELECT  `id`, MATCH (`title` ,  `description`, `requirements` ,  `restrictions` ,  `samples`)
            ...
            FROM " . $wpdb->prefix . "gs_grants WHERE id in ($this->newsletter_test_ids) ORDER BY score DESC";
    ...
    function search_by_keyword( $keyword )
    {
        ...
        $sql_query = "SELECT  `id`, MATCH (`title` ,  `description`, `requirements` ,  `restrictions` ,  `samples`)
            ...
            AGAINST (%s IN BOOLEAN MODE) AND id in ($this->newsletter_ids) ORDER BY score DESC";
    ...
    function search_by_geo_location( $geo_ids )
    {
        ...
        $sql_query = "SELECT grant_id FROM " . $wpdb->prefix . "gs_grant_geo_mappings as ggm
                        ...
                        WHERE geo_id IN($geo_ids) AND gg.id in ($this->newsletter_ids)";
    ...
    function search_by_subject_title( $subject_title_ids )
    {
        ...
        $sql_query = "SELECT grant_id FROM " . $wpdb->prefix . "gs_grant_subject_mappings as gsm
                        ...
                        WHERE subject_id IN($subject_title_ids) AND gg.id in ($this->newsletter_ids)";
    ...
    function search_by_program_type( $program_ids )
    {
        ...
        $sql_query = "SELECT grant_id FROM " . $wpdb->prefix . "gs_grant_program_mappings as gpm
                        ...
                        WHERE program_id IN ($program_ids) AND gg.id in ($this->newsletter_ids)";
    ...
    /**
     * Function match user segment to out array.
     * @params $out_array, $subscr_id
     * @return $out_array 
     */
    private function match_user_segment($main, $subscr_id) 
    {
        global $wpdb;
        if (empty($main)){
            return $main;
        ...
        $res_array = array();
        $subscr_info = $wpdb->get_row($wpdb->prepare("select subscription_plan_id from {$wpdb->prefix}pms_member_subscriptions where id=%d", array($subscr_id)));
        if (!isset(GS_NEWSLETTER_SUBSCRIPTION_PLANS[$subscr_info->subscription_plan_id]))
            return $main;
        $segment = GS_NEWSLETTER_SUBSCRIPTION_PLANS[$subscr_info->subscription_plan_id];
        $grant_ids = implode(",", $main);
        $result = $wpdb->get_results($wpdb->prepare("select grant_id from {$wpdb->prefix}gs_grant_segment_mappings where segment_id=%d and grant_id in ($grant_ids)", array($segment)));
    ...
    private function send_mail_to_subscr($grant_info, $subscr_email)
    {
        ...
        //echo $email_body;
        wp_mail($subscr_email, $this->subject, $email_body, $headers);
    }
    /**
    ...
    private function make_email_body($grant_info)
    {
        ...
        $admin_email = get_bloginfo('admin_email');
        $home_url = home_url();
        $article = get_option("ns_article")==false?"":get_option("ns_article");
        $article = str_replace(array("\r\n", "\r", "\n"), "<br>\n", $article);
        $article = str_ireplace("</h1><br>","</h1>",$article);
        $article = str_ireplace("</h2><br>","</h2>",$article);
        $article = str_ireplace("</h3><br>","</h3>",$article);
        $email_body = <<< EndOfMailHeader
        <head>
	
	<title>Grant Watch Newsletter</title>
                  ...
                  <img src="$base_url/images/breaker.jpg" height="20" alt="" style="border: 0;" width="600" />
							...
							<tr>
								<td class="header-bar" valign="top" style="color: #999; font-family: Verdana; font-size: 10px; text-transform: uppercase; padding: 0 20px" width="600">
                                    A Product of the Practical Online Grants Resource | <a href="$home_url" style="text-decoration:none;color:#999">GrantSelect.com</a>
								</td>
                            </tr>
							<tr>
...
EndOfMailHeader;
        ...
        if ( !empty($article) ) {
            $email_body .= <<< EndOfMailArticle
                            <tr>
                                <td style="padding: 0 20px; valign="top" width="600" colspan="2">
                                $article
                                <br>
                                <br>
                                </td>
					...
					<td valign="top" width="600">
						...
						<table cellspacing="0" border="0" height="202" cellpadding="0" width="600">
							...
							<tr>
                                ...
                                <p><span style="padding: 20px 0; color: #90B72E; font-family: Georgia; font-size: 14px; font-weight: bold;">Modify Newsletter Settings</span><br />
                                You can modify your settings by logging on at <a href="$home_url/login/" style="color: #9E3B00; text-decoration: none;">Grant Watch</a>. Change as many parameters (keywords, subjects, program types, geographic locations) as needed in order to receive grant opportunities in those areas.</p>
                                </td>
							</tr>
							<tr>
