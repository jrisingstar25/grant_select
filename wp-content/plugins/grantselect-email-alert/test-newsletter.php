...
class GS_Test_Newsletter extends GS_Cron_Newsletter{
    ...
    public function send($limit='all'){
        ...
        $ea_id = 0;
        $emails = "";
        $test_emails = explode(",", $_POST['test_emails']);
        foreach ($test_emails as $te){
            if ($emails == ""){
                $emails.= "'" . $te . "'";
            }else{
                $emails.=",'" . $te . "'";
