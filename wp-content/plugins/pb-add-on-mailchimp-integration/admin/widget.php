    ...
    class WPPB_Mailchimp_Widget extends WP_Widget {
        ...
        public function widget( $args, $instance ) {
            ...
            if( ( !$wppb_mci_api_key_validated || empty( $wppb_mci_api_key ) ) && current_user_can( 'manage_options' ) ) {
                ...
                echo __( 'Something went wrong. Either the MailChimp API key is missing or it is invalid.', 'profile-builder' );
            ...
            // If API key is okay display the form
                ...
                if( $validate_response['success'] == 1 ) {
                    ...
                    $response = wppb_mci_api_subscribe( $wppb_mci_api_key, $instance['list_id'], $validate_response['subscribe_arguments'] );
                    if ( !empty( $_POST['widget_mailchimp_gdpr_' . $instance['list_id']] ) && $response['id'])
                        wppb_mci_gdpr_opt_in( $wppb_mci_api_key, $instance['list_id'], $response['id'] );

                    if( isset( $response['id'] ) ) {

                        echo '<div class="wppb-mci-widget-subscribe-success-wrapper">';
                            if( $instance['double_opt_in'] == 'on' )
                                echo '<p class="wppb-mci-widget-subscribe-success-message">' . sprintf( __( '%s', 'profile-builder' ), apply_filters( 'wppb_mci_widget_subscribe_new_success_double_opt_in', 'A confirmation email has been sent to the e-mail address you have provided' ) ) . '</p>';
                            else
                                echo '<p class="wppb-mci-widget-subscribe-success-message">' . sprintf( __( '%s', 'profile-builder' ), apply_filters( 'wppb_mci_widget_subscribe_new_success', 'You have been subscribed to our newsletter' ) ) . '</p>';
                        echo '</div>';

                    }
                    else{
                        echo '<p class="wppb-mci-widget-subscribe-error-message">' . sprintf( __( '%s', 'profile-builder' ), apply_filters( 'wppb_mci_widget_subscribe_new_success', 'You have been subscribed to our newsletter' ) ) . '</p>';
                    }
                    ...
                    if (!empty($instance['list_id']) && !empty($instance['lists_fields'])) {
                        ...
                        if( isset( $instance['gdpr'] ) && $instance['gdpr'] == 'on' ){
                            echo '<label for="widget_mailchimp_gdpr_' . $instance['list_id'] . '">';
                            echo '<input name="widget_mailchimp_gdpr_' . $instance['list_id'] . '" id="widget_mailchimp_gdpr_' . $instance['list_id'] . '" class="extra_field_mailchimp" type="checkbox" value="true" />';
                            echo __(  apply_filters( 'wppb_mci_gdpr_text', 'By checking this box I consent to the use of my information provided for email marketing purposes.'), 'profile-builder') . '</label>';
                        }

                        // Display the submit button
                        echo '<input type="submit" value="' . apply_filters('wppb_mci_widget_submit_button_text', __('Submit', 'profile-builder')) . '" />';
        ...
        public function form( $instance ) {
            ...
            if( get_option('wppb_mailchimp_api_key_validated', false) && !empty( $wppb_mci_api_key ) ) {
                ...
                <div class="wppb-mci-widget-wrapper">
                    <label class="wppb-mci-widget-label" for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title', 'profile-builder' ); ?></label>
                    <div class="wppb-mci-widget-input-wrapper">
                        <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>">
                    </div>

                    <label class="wppb-mci-widget-label" for="wppb_mci_widget_list_select"><?php _e( 'Select the list your new subscriber will be added to', 'profile-builder' ); ?></label><br />
                    <div class="wppb-mci-widget-input-wrapper">
                        ...
                        <option value="0"><?php echo __( 'Select...', 'profile-builder' ); ?></option>
                        <?php
                    ...
                    <label class="wppb-mci-widget-label"><?php echo __( 'Select which fields to show', 'profile-builder' ); ?></label>
                    ...
                    <label class="wppb-mci-widget-label"><?php echo __( 'Extra Options', 'profile-builder' ); ?></label>
                    <div class="wppb-mci-widget-input-wrapper">

                        <div class="wppb-mci-widget-setting-double-opt-in">
                            <label><input type="checkbox" name="<?php echo $this->get_field_name('double_opt_in'); ?>" <?php checked( $double_opt_in, 'on' ) ?> /><?php echo __( 'Double Opt-In', 'profile-builder' ); ?></label>
                            <p class="description"><?php echo __( 'If you select double opt-in, the user will receive an email to confirm the subscription', 'profile-builder' ); ?></p>
                        </div>
                        ...
                        <div class="wppb-mci-widget-setting-gdpr <?php echo $hidden_class; ?>">
                            <label><input type="checkbox" name="<?php echo $this->get_field_name('gdpr'); ?>" <?php checked( $gdpr, 'on' ) ?> /><?php echo __( 'Enable GDPR', 'profile-builder' ); ?></label>
                            <p class="description"><?php echo sprintf( __( 'If checked will enable GDPR on this list. <a href="%s" target="_blank">You must also enable GDPR on the list from mailchimp</a>', 'profile-builder' ), 'https://mailchimp.com/help/collect-consent-with-gdpr-forms/#Set_Up_Your_GDPR-Friendly_Signup_Form' ) ?></p>
                        </div>
                    </div>
                </div>
        ...
        public function display_list_fields( $list_id = '', $widget_number = '', $is_ajax = false ) {
            ...
            if( !empty($list_id) && !empty( $widget_number ) ) {
                    ...
                    foreach ($merge_vars as $merge_var) {
                        ...
                        if ($merge_var['tag'] == 'EMAIL') {
                            ...
                            if ($merge_var['tag'] == 'FNAME' || $merge_var['tag'] == 'LNAME') {
                                ...
                                echo '<input type="hidden" name="widget-' . $this->id_base . '[' . $widget_number . '][lists_fields]' . '[' . $field_key . '][required]" value="' . $merge_var_req . '" />';
                ...
                echo '<span class="wppb-mci-widget-span-message">' . __( 'Please select a list first', 'profile-builder') . '</span>';
