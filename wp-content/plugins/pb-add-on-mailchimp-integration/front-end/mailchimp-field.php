        ...
        if ( isset( $settings['api_key'] ) && !empty($settings['api_key']) && $key_valid != false ) {
            ...
            if( $form_location != 'back_end' ) {
                ...
                if( isset( $settings['lists'][$list_id]['gdpr'] ) && $settings['lists'][$list_id]['gdpr'] == 'on' ){
                    ...
                    $output .= '<label for="custom_field_mailchimp_gdpr_' . $field['id'] . '">';
                    $output .= '<input name="custom_field_mailchimp_gdpr_' . $field['id'] . '" id="custom_field_mailchimp_gdpr_' . $field['id'] . '" class="extra_field_mailchimp" type="checkbox" value="true" ' . $gdpr_checked . ' />';
                    $output .= __( apply_filters( 'wppb_mci_gdpr_text', 'By checking this box I consent to the use of my information provided for email marketing purposes.' ), 'profile-builder') . '</label>';
                }
        ...
        if( !empty( $request_data['custom_field_mailchimp_subscribe_' . $field['id']] ) ) {
            ...
            if( isset( $settings['api_key'] ) && !empty( $settings['api_key'] ) && $key_valid != false && !empty($mc_list_id) ) {
                ...
                if( empty( $form_location ) || $form_location == 'register' ) {
	                wppb_mci_api_unsubscribe( $settings['api_key'], array( 'id' => $mc_list_id, 'email' => wppb_mci_api_get_args_email( $request_data, $user_id, $form_location ) ) );
                    $response = wppb_mci_api_subscribe($settings['api_key'], $mc_list_id, wppb_mci_api_get_args('subscribe', $request_data, $mc_list_id, $user_id, $form_location));
                    if ( !empty( $request_data['custom_field_mailchimp_gdpr_' . $field['id']] ) && $response['id'])
                        wppb_mci_gdpr_opt_in($settings['api_key'], $mc_list_id, $response['id']);
                }
                ...
                // Handle subscription / updating member on edit profile
                    ...
                    if( $is_subscribed ) {
                        ...
                        if( $old_user_email !== false ) {
                            ...
                            wppb_mci_api_unsubscribe( $settings['api_key'], array( 'id' => $mc_list_id, 'email' => $old_user_email ) );
                            $response = wppb_mci_api_subscribe( $settings['api_key'], $mc_list_id, wppb_mci_api_get_args( 'update-member', $request_data, $mc_list_id, $user_id, $form_location ) );
                            if ( !empty( $request_data['custom_field_mailchimp_gdpr_' . $field['id']] ) && $response['id'])
                                wppb_mci_gdpr_opt_in($settings['api_key'], $mc_list_id, $response['id']);
                            else
                                wppb_mci_gdpr_opt_out($settings['api_key'], $mc_list_id, $response['id']);

                        } else {
                            $response = wppb_mci_api_update_member($settings['api_key'], $mc_list_id, wppb_mci_api_get_args('update-member', $request_data, $mc_list_id, $user_id, $form_location));
                            if ( !empty( $request_data['custom_field_mailchimp_gdpr_' . $field['id']] ) && $response['id'])
                                wppb_mci_gdpr_opt_in($settings['api_key'], $mc_list_id, $response['id']);
                            else
                                wppb_mci_gdpr_opt_out($settings['api_key'], $mc_list_id, $response['id']);
            ...
            if( isset( $settings['api_key'] ) && !empty( $settings['api_key'] ) && $key_valid != false ) {
                ...
                if (isset($user_data) && !empty($user_data)) {
                    ...
                    wppb_mci_api_unsubscribe( $settings['api_key'], array( 'id' => $mc_list_id, 'email' => $unsubscribe_email ) );
        ...
        foreach( $wppb_manage_fields as $field ) {
            ...
            if( $field['field'] == 'MailChimp Subscribe' && isset( $meta[ 'custom_field_mailchimp_subscribe_' . $field['id'] ] ) && !empty( $meta[ 'custom_field_mailchimp_subscribe_' . $field['id'] ] ) ) {
                ...
                if( isset( $settings['api_key'] ) && !empty( $settings['api_key'] ) && $key_valid != false && !empty($mc_list_id) ) {
                    ...
                    // Get the args
                    $args = wppb_mci_api_get_args( 'subscribe', $meta, $mc_list_id, $user_id );

                    // Subscribe new
                    $response = wppb_mci_api_subscribe( $settings['api_key'], $mc_list_id, $args );
