 ...
 */
define( 'PMS_POTX_STATUS_SILENT', 0 );

/**
 * Drupal message based status reports.
 */
define( 'PMS_POTX_STATUS_MESSAGE', 1 );
 ...
 */
define( 'PMS_POTX_STATUS_STRUCTURED', 3 );
 ...
 */
function _pms_potx_process_file ( $file_path, $strip_prefix = 0, $save_callback = '_pms_potx_save_string', $version_callback = '_pms_potx_save_version', $api_version = PMS_POTX_API_6 )
{
    ...
    _pms_potx_find_version_number ( $code, $file_name, $version_callback );
    ...
    if ( !empty( $src_tokens ) )
        foreach ( $src_tokens as $tk ) {
            _pms_potx_find_t_calls_with_context ( $file_name, $save_callback, $tk );
        }
 ...
 *
 * @param $string_mode
 *   Strings to generate files for: PMS_POTX_STRING_RUNTIME or PMS_POTX_STRING_INSTALLER.
 * @param $build_mode
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_build_files ( $string_mode = PMS_POTX_STRING_RUNTIME, $build_mode = PMS_POTX_BUILD_SINGLE, $force_name = 'general', $save_callback = '_pms_potx_save_string', $version_callback = '_pms_potx_save_version', $header_callback = '_pms_potx_get_header', $template_export_langcode = NULL, $translation_export_langcode = NULL, $api_version = PMS_POTX_API_6 )
{
    ...
    foreach ( $strings as $string => $string_info ) {
        foreach ( $string_info as $context => $file_info ) {
            ...
            if ( $build_mode == PMS_POTX_BUILD_SINGLE ) {
                ...
                $file_name = ( $build_mode == PMS_POTX_BUILD_CORE ? 'general' : str_replace ( '.info', '.module', $file_name ) );
            ...
            if ( strpos ( $string, "\0" ) !== FALSE ) {
                ...
                if ( isset( $translation_export_langcode ) ) {
                    $output .= _pms_potx_translation_export ( $translation_export_langcode, $singular, $plural, $api_version );
                } else {
                    $output .= "msgstr[0] \"\"\n";
                    $output .= "msgstr[1] \"\"\n";
                }
                ...
                if ( isset( $translation_export_langcode ) ) {
                    $output .= _pms_potx_translation_export ( $translation_export_langcode, $string, NULL, $api_version );
                } else {
                    $output .= "msgstr \"\"\n";
                }
            }
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_translation_export ( $translation_export_langcode, $string, $plural = NULL, $api_version = PMS_POTX_API_6 )
{
    ...
    $language_column = $api_version > PMS_POTX_API_5 ? 'language' : 'locale';
    $language_table = $api_version > PMS_POTX_API_5 ? 'languages' : 'locales_meta';
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_get_header ( $file, $template_export_langcode = NULL, $api_version = PMS_POTX_API_6 )
{
    // We only have language to use if we should export with that langcode.
    $language = NULL;
    if ( isset( $template_export_langcode ) ) {
        $language = db_fetch_object ( db_query ( $api_version > PMS_POTX_API_5 ? "SELECT language, name, plurals, formula FROM {languages} WHERE language = '%s'" : "SELECT locale, name, plurals, formula FROM {locales_meta} WHERE locale = '%s'", $template_export_langcode ) );
    }
 ...
 */
function _pms_potx_write_files ( $http_filename = NULL, $content_disposition = 'inline' )
{
    global $_pms_potx_store;
 ...
 */
function _pms_potx_format_quoted_string ( $str )
{
    $quo = substr ( $str, 0, 1 );
    $str = substr ( $str, 1, -1 );
    if ( $quo == '"' ) {
 ...
 */
function _pms_potx_marker_error ( $file, $line, $marker, $ti, $error, $docs_url = NULL )
{
    ...
    pms_potx_status ( 'error', $error, $file, $line, $marker . '(' . $tokens, $docs_url );
}
 ...
 */
function pms_potx_status ( $op, $value = NULL, $file = NULL, $line = NULL, $excerpt = NULL, $docs_url = NULL )
{
    static $mode = PMS_POTX_STATUS_CLI;
    ...
    switch ( $op ) {
        ...
        case 'status':
            ...
            $location_info = '';
            if ( ( $mode != PMS_POTX_STATUS_STRUCTURED ) && isset( $file ) ) {
                if ( isset( $line ) ) {
                    if ( isset( $excerpt ) ) {
            ...
            $read_more = '';
            if ( ( $mode != PMS_POTX_STATUS_STRUCTURED ) && isset( $docs_url ) ) {
                $read_more = ( $mode == PMS_POTX_STATUS_CLI ) ? t ( 'Read more at @url', array( '@url' => $docs_url ) ) : t ( 'Read more at <a href="@url">@url</a>', array( '@url' => $docs_url ) );
            }

            // Error message or progress text to display.
            switch ( $mode ) {
                case PMS_POTX_STATUS_MESSAGE:
                    drupal_set_message ( join ( ' ', array( $value, $location_info, $read_more ) ), $op );
                    break;
                case PMS_POTX_STATUS_CLI:
                    if ( defined ( 'STDERR' ) && defined ( 'STDOUT' ) ) {
                        fwrite ( $op == 'error' ? STDERR : STDOUT, join ( "\n", array( $value, $location_info, $read_more ) ) . "\n\n" );
                    }
                    break;
                case PMS_POTX_STATUS_SILENT:
                    if ( $op == 'error' ) {
                        $messages[ ] = join ( ' ', array( $value, $location_info, $read_more ) );
                    }
                    break;
                case PMS_POTX_STATUS_STRUCTURED:
                    if ( $op == 'error' ) {
 ...
 * @param $string_mode
 *   String mode to use: PMS_POTX_STRING_INSTALLER, PMS_POTX_STRING_RUNTIME or
 *   PMS_POTX_STRING_BOTH.
 */
function _pms_potx_find_t_calls ( $file, $save_callback, $function_name = 't', $string_mode = PMS_POTX_STRING_RUNTIME )
{
    global $_pms_potx_tokens, $_pms_potx_lookup;
        ...
        foreach ( $_pms_potx_lookup[ $function_name ] as $ti ) {
            ...
            if ( $par == "(" ) {
                if ( in_array ( $rig, array( ")", "," ) )
                    ...
                    $save_callback( _pms_potx_format_quoted_string ( $mid[ 1 ] ), PMS_POTX_CONTEXT_NONE, $file, $line, $string_mode );
                } else {
                    // $function_name() found, but inside is something which is not a string literal.
                    _pms_potx_marker_error ( $file, $line, $function_name, $ti, t ( 'The first parameter to @function() should be a literal string. There should be no variables, concatenation, constants or other non-literal strings there.', array( '@function' => $function_name ) ), 'http://drupal.org/node/322732' );
                }
            }
        }
    }
 ...
 *   PMS_POTX_STRING_BOTH.
 */
function _pms_potx_find_t_calls_with_context ( $file, $save_callback, $function_name = '_e', $string_mode = PMS_POTX_STRING_RUNTIME )
{
    global $_pms_potx_tokens, $_pms_potx_lookup;
        ...
        foreach ( $_pms_potx_lookup[ $function_name ] as $ti ) {
            ...
            if ( $par == "(" ) {
                if ( in_array ( $rig, array( ")", "," ) )
                    ...
                    if ( $domain !== PMS_POTX_CONTEXT_ERROR ) {
                        // Only save if there was no error in context parsing.
                        $save_callback( _pms_potx_format_quoted_string ( $mid[ 1 ] ), $domain, @strval ( $context ), $file, $line, $string_mode );
                        if ( isset( $text_plural ) ) {
                            $save_callback( _pms_potx_format_quoted_string ( $text_plural ), $domain, $context, $file, $line, $string_mode );
                        }
                    }
                } else {
                    // $function_name() found, but inside is something which is not a string literal.
 ...
 *   Callback function used to save strings.
 */
function _pms_potx_find_watchdog_calls ( $file, $save_callback )
{
    global $_pms_potx_tokens, $_pms_potx_lookup;
        ...
        foreach ( $_pms_potx_lookup[ 'watchdog' ] as $ti ) {
            ...
            if ( $par == '(' ) {
                ...
                if ( in_array ( $rig, array( ')', ',' ) ) && $comma == ','
                    ...
                    $save_callback( _pms_potx_format_quoted_string ( $mtype[ 1 ] ), PMS_POTX_CONTEXT_NONE, $file, $line );
                    $save_callback( _pms_potx_format_quoted_string ( $message[ 1 ] ), PMS_POTX_CONTEXT_NONE, $file, $line );
                } else {
                    // watchdog() found, but inside is something which is not a string literal.
                    _pms_potx_marker_error ( $file, $line, 'watchdog', $ti, t ( 'The first two watchdog() parameters should be literal strings. There should be no variables, concatenation, constants or even a t() call there.' ), 'http://drupal.org/node/323101' );
                }
            }
        }
    }
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_find_format_plural_calls ( $file, $save_callback, $api_version = PMS_POTX_API_6 )
{
    global $_pms_potx_tokens, $_pms_potx_lookup;
    ...
    if ( isset( $_pms_potx_lookup[ 'format_plural' ] ) ) {
        foreach ( $_pms_potx_lookup[ 'format_plural' ] as $ti ) {
            ...
            if ( $par1 == "(" ) {
                ...
                if ( ( $comma2 == ',' ) && ( $par2 == ')' || ( $par2 == ',' && $api_version > PMS_POTX_API_5 ) ) &&
                    ( is_array ( $singular ) && ( $singular[ 0 ] == T_CONSTANT_ENCAPSED_STRING ) ) &&
                    ( is_array ( $plural ) && ( $plural[ 0 ] == T_CONSTANT_ENCAPSED_STRING ) )
                ) {
                    // By default, there is no context.
                    $context = PMS_POTX_CONTEXT_NONE;
                    if ( $par2 == ',' && $api_version > PMS_POTX_API_6 ) {
                        // If there was a comma after the plural, we need to look forward
                        // to try and find the context.
                        $context = _pms_potx_find_context ( $ti, $tn + 5, $file, 'format_plural' );
                    }
                    if ( $context !== PMS_POTX_CONTEXT_ERROR ) {
                        // Only save if there was no error in context parsing.
                        $save_callback(
                            _pms_potx_format_quoted_string ( $singular[ 1 ] ) . "\0" . _pms_potx_format_quoted_string ( $plural[ 1 ] ),
                            $context,
                            $file,
                            $line
                        );
                    ...
                    _pms_potx_marker_error ( $file, $line, "format_plural", $ti, t ( 'In format_plural(), the singular and plural strings should be literal strings. There should be no variables, concatenation, constants or even a t() call there.' ), 'http://drupal.org/node/323072' );
                }
            }
        }
    }
 ...
 *   Callback function used to save strings.
 */
function _pms_potx_find_perm_hook ( $file, $filebase, $save_callback )
{
    global $_pms_potx_tokens, $_pms_potx_lookup;
    ...
    if ( isset( $_pms_potx_lookup[ $filebase . '_perm' ] ) ) {
        ...
        if ( $filebase == 'node' ) {
            ...
            foreach ( $nodeperms as $item ) {
                ...
                // support context.
                $save_callback( $item, PMS_POTX_CONTEXT_NONE, $file, $line );
            }
        } else {
            $count = 0;
            foreach ( $_pms_potx_lookup[ $filebase . '_perm' ] as $ti ) {
                ...
                while ( is_array ( $_pms_potx_tokens[ $tn ] ) || $_pms_potx_tokens[ $tn ] != '}' ) {
                    if ( is_array ( $_pms_potx_tokens[ $tn ] ) && $_pms_potx_tokens[ $tn ][ 0 ] == T_CONSTANT_ENCAPSED_STRING ) {
                        // hook_perm() is only ever found on a Drupal system which does not
                        // support context.
                        $save_callback( _pms_potx_format_quoted_string ( $_pms_potx_tokens[ $tn ][ 1 ] ), PMS_POTX_CONTEXT_NONE, $file, $_pms_potx_tokens[ $tn ][ 2 ] );
                        $count++;
                    }
                    $tn++;
                }
            ...
            if ( !$count ) {
                pms_potx_status ( 'error', t ( '%hook should have an array of literal string permission names.', array( '%hook' => $filebase . '_perm()' ) ), $file, NULL, NULL, 'http://drupal.org/node/323101' );
            }
        }
    }
}
 ...
 */
function _pms_potx_find_end_of_function ( $here )
{
    global $_pms_potx_tokens;
 ...
 */
function _pms_potx_skip_args ( $here )
{
    global $_pms_potx_tokens;
        ...
        if ( $com == ',' && $arr[ 1 ] == 'array' && $par == '(' ) {
            ...
            if ( $nesting == 0 ) {
                ...
                if ( is_array ( $arw ) && $arw[ 1 ] == '=>' && is_array ( $str ) && $str[ 0 ] == T_CONSTANT_ENCAPSED_STRING ) {
                    return _pms_potx_format_quoted_string ( $str[ 1 ] );
                } else {
                    list( $type, $string, $line ) = $_pms_potx_tokens[ $ti ];
                    // @todo: fix error reference.
                    _pms_potx_marker_error ( $file, $line, $function_name, $tf, t ( 'The context element in the options array argument to @function() should be a literal string. There should be no variables, concatenation, constants or other non-literal strings there.', array( '@function' => $function_name ) ), 'http://drupal.org/node/322732' );
                    // Return with error.
                    return PMS_POTX_CONTEXT_ERROR;
                }
            } else {
                // Did not found 'context' key in $options array.
                return PMS_POTX_CONTEXT_NONE;
            }
        }
    }
 ...
 *   Callback function used to save strings.
 */
function _pms_potx_find_menu_hook ( $file, $filebase, $save_callback )
{
    global $_pms_potx_tokens, $_pms_potx_lookup;
    ...
    if ( isset( $_pms_potx_lookup[ $filebase . '_menu' ] ) && is_array ( $_pms_potx_lookup[ $filebase . '_menu' ] ) ) {
        ...
        foreach ( $_pms_potx_lookup[ $filebase . '_menu' ] as $ti ) {
            $end = _pms_potx_find_end_of_function ( $ti );
            $tn = $ti;
            while ( $tn < $end ) {
                ...
                if ( $_pms_potx_tokens[ $tn ][ 0 ] == T_CONSTANT_ENCAPSED_STRING && in_array ( $_pms_potx_tokens[ $tn ][ 1 ], array( "'title'", '"title"', "'description'", '"description"' ) ) && $_pms_potx_tokens[ $tn + 1 ][ 0 ] == T_DOUBLE_ARROW ) {
                    if ( $_pms_potx_tokens[ $tn + 2 ][ 0 ] == T_CONSTANT_ENCAPSED_STRING ) {
                        ...
                        $save_callback(
                            _pms_potx_format_quoted_string ( $_pms_potx_tokens[ $tn + 2 ][ 1 ] ),
                            PMS_POTX_CONTEXT_NONE,
                            $file,
                            $_pms_potx_tokens[ $tn + 2 ][ 2 ]
                        );
                        $tn += 2; // Jump forward by 2.
                    } else {
                        pms_potx_status ( 'error', t ( 'Invalid menu %element definition found in %hook. Title and description keys of the menu array should be literal strings.', array( '%element' => $_pms_potx_tokens[ $tn ][ 1 ], '%hook' => $filebase . '_menu()' ) ), $file, $_pms_potx_tokens[ $tn ][ 2 ], NULL, 'http://drupal.org/node/323101' );
                    }
                }
                $tn++;
            }
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_find_language_names ( $file, $save_callback, $api_version = PMS_POTX_API_6 )
{
    global $_pms_potx_tokens, $_pms_potx_lookup;

    foreach ( $_pms_potx_lookup[ $api_version > PMS_POTX_API_5 ? '_locale_get_predefined_list' : '_locale_get_iso639_list' ] as $ti ) {
    ...
    while ( $ti < $end ) {
        ...
        $save_callback( _pms_potx_format_quoted_string ( $_pms_potx_tokens[ $ti ][ 1 ] ), PMS_POTX_CONTEXT_NONE, $file, $_pms_potx_tokens[ $ti ][ 2 ] );
    }
}
 ...
 */
function _pms_potx_find_version_number ( $code, $file, $version_callback )
{
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_add_date_strings ( $file, $save_callback, $api_version = PMS_POTX_API_6 )
{
    for ( $i = 1; $i <= 12; $i++ ) {
        $stamp = mktime ( 0, 0, 0, $i, 1, 1971 );
        if ( $api_version > PMS_POTX_API_6 ) {
            // From Drupal 7, long month names are saved with this context.
            $save_callback( date ( "F", $stamp ), 'Long month name', $file );
        } elseif ( $api_version > PMS_POTX_API_5 ) {
            // Drupal 6 uses a little hack. No context.
            $save_callback( '!long-month-name ' . date ( "F", $stamp ), PMS_POTX_CONTEXT_NONE, $file );
        } else {
            // Older versions just accept the confusion, no context.
            $save_callback( date ( "F", $stamp ), PMS_POTX_CONTEXT_NONE, $file );
        }
        // Short month names lack a context anyway.
        $save_callback( date ( "M", $stamp ), PMS_POTX_CONTEXT_NONE, $file );
    }
    for ( $i = 0; $i <= 7; $i++ ) {
        $stamp = $i * 86400;
        $save_callback( date ( "D", $stamp ), PMS_POTX_CONTEXT_NONE, $file );
        $save_callback( date ( "l", $stamp ), PMS_POTX_CONTEXT_NONE, $file );
    }
    $save_callback( 'am', PMS_POTX_CONTEXT_NONE, $file );
    $save_callback( 'pm', PMS_POTX_CONTEXT_NONE, $file );
    $save_callback( 'AM', PMS_POTX_CONTEXT_NONE, $file );
    $save_callback( 'PM', PMS_POTX_CONTEXT_NONE, $file );
}
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_add_format_interval_strings ( $file, $save_callback, $api_version = PMS_POTX_API_6 )
{
    $components = array(
        '1 year' => '@count years',
        '1 week' => '@count weeks',
    ...
    foreach ( $components as $singular => $plural ) {
        ...
        $save_callback( $singular . "\0" . $plural, PMS_POTX_CONTEXT_NONE, $file );
    }
}
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_add_default_region_names ( $file, $save_callback, $api_version = PMS_POTX_API_6 )
{
    $regions = array(
        'left' => 'Left sidebar',
        'right' => 'Right sidebar',
    ...
    foreach ( $regions as $region ) {
        ...
        $save_callback( $region, PMS_POTX_CONTEXT_NONE, $file );
    }
}
 ...
 *   Drupal API version to work with.
 */
function _pms_potx_find_info_file_strings ( $file_path, $file_name, $save_callback, $api_version = PMS_POTX_API_6 )
{
    $info = array();

    if ( file_exists ( $file_path ) ) {
        $info = $api_version > PMS_POTX_API_5 ? drupal_parse_info_file ( $file_path ) : parse_ini_file ( $file_path );
    }
    ...
    foreach ( array( 'name', 'description', 'package' ) as $key ) {
        if ( isset( $info[ $key ] ) ) {
            ...
            $save_callback( $info[ $key ], PMS_POTX_CONTEXT_NONE, $file_name );
        }
    }
    ...
    if ( isset( $info[ 'regions' ] ) && is_array ( $info[ 'regions' ] ) ) {
        foreach ( $info[ 'regions' ] as $region => $region_name ) {
            // No context support for .info file strings.
            $save_callback( $region_name, PMS_POTX_CONTEXT_NONE, $file_name );
        }
    }
}
 ...
 */
function _pms_potx_parse_js_file ( $code, $file, $save_callback )
{
    $js_string_regex = '(?:(?:\'(?:\\\\\'|[^\'])*\'|"(?:\\\\"|[^"])*")(?:\s*\+\s*)?)+';
    ...
    if ( isset( $t_matches ) && count ( $t_matches ) ) {
        foreach ( $t_matches as $match ) {
            ...
            $save_callback( _pms_potx_parse_js_string ( $match[ 1 ] ), PMS_POTX_CONTEXT_NONE, $file, 0 );
        }
    }
    ...
    if ( isset( $plural_matches ) && count ( $plural_matches ) ) {
        foreach ( $plural_matches as $index => $match ) {
            ...
            // @todo: figure out how to parse out context, once Drupal supports it.
            $save_callback(
                _pms_potx_parse_js_string ( $match[ 1 ] ) . "\0" . _pms_potx_parse_js_string ( $match[ 2 ] ),
                PMS_POTX_CONTEXT_NONE,
                $file,
                0
            );
    ...
    if ( isset( $faulty_matches ) && count ( $faulty_matches ) ) {
        foreach ( $faulty_matches as $index => $match ) {
            ...
            pms_potx_status ( 'error', $message, $file, NULL, $match[ 0 ], 'http://drupal.org/node/323109' );
        }
    }
}
 ...
 */
function _pms_potx_parse_js_string ( $string )
{
    return _pms_potx_format_quoted_string ( implode ( '', preg_split ( '~(?<!\\\\)[\'"]\s*\+\s*[\'"]~s', $string ) ) );
}
 ...
 */
function _pms_potx_explore_dir ( $path = '', $basename = '*', $api_version = PMS_POTX_API_6 )
{
    ...
    if ( is_array ( $dirs ) ) {
        foreach ( $dirs as $dir ) {
            if ( !preg_match ( "!(^|.+/)(CVS|.svn|.git)$!", $dir ) ) {
                $files = array_merge ( $files, _pms_potx_explore_dir ( "$dir/", $basename ) );
            }
        }
    }
 ...
 */
function _pms_potx_save_version ( $value = NULL, $file = NULL )
{
 ...
 * @param $context
 *   From Drupal 7, separate contexts are supported. PMS_POTX_CONTEXT_NONE is
 ...
 * @param $string_mode
 *   String mode: PMS_POTX_STRING_INSTALLER, PMS_POTX_STRING_RUNTIME
 *   or PMS_POTX_STRING_BOTH.
 */
function _pms_potx_save_string ( $value = NULL, $context = NULL, $file = NULL, $line = 0, $string_mode = PMS_POTX_STRING_RUNTIME )
{
    global $_pms_potx_strings, $_pms_potx_install;

	if ( isset( $value ) ) {
        switch ( $string_mode ) {
            case PMS_POTX_STRING_BOTH:
            ...
            case PMS_POTX_STRING_RUNTIME:
                // Mark runtime strings as duplicates of installer strings if
                // the string was both recorded in the runtime and in the installer.
                $_pms_potx_strings[ $value ][ $context ][ $file ][ ] = $line . ( $string_mode == PMS_POTX_STRING_BOTH ? ' (dup)' : '' );
                break;
            case PMS_POTX_STRING_INSTALLER:
                $_pms_potx_install[ $value ][ $context ][ $file ][ ] = $line;
                break;
        }
    } else {
        return ( $string_mode == PMS_POTX_STRING_RUNTIME ? $_pms_potx_strings : $_pms_potx_install );
    }
}
