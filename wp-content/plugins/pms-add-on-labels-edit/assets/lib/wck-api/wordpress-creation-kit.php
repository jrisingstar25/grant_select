...
new Wordpress_Creation_Kit_PMSLE( $args );
...
class Wordpress_Creation_Kit_PMSLE{
	...
	function wck_output_form_field( $meta, $details, $value = '', $context = '', $post_id = '' ){
		...
		if( $context == 'edit_form' ){
            ...
            if( isset( $details['default'] ) && !( $this->args['single'] == true && !is_null( $value ) ) ) {
                $value = apply_filters("wck_default_value_{$meta}_" . Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details ), $details['default']);
            }
        }
        ...
        $element .= '<label for="'. $single_prefix . esc_attr( Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'],  $details ) ) .'" class="field-label">'. apply_filters( "wck_label_{$meta}_". Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details ), ucfirst($details['title']) ) .':';
		...
		*/
		...
		$element = apply_filters( "wck_output_form_field_{$meta}_" . Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details ), $element );
			...
			<ul class="mb-list-entry-fields">
				...
				if( !empty( $fields ) ){
					foreach( $fields as $details ){
                        ...
                        if( $this->args['single'] == true ) {
                            $value = null;
                            if (isset($results[0][Wordpress_Creation_Kit_PMSLE::wck_generate_slug($details['title'], $details )]))
                                $value = $results[0][Wordpress_Creation_Kit_PMSLE::wck_generate_slug($details['title'], $details )];
                        }
                        ?>
							<li class="row-<?php echo esc_attr( Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details ) ) ?>">
		...
		if($results != null){
			...
			if( !empty( $fields ) ){
				foreach( $fields as $field ){
					$details = $field;
					if( isset( $results[$element_id][Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details )] ) )
						$value = $results[$element_id][Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details )];
					else
						$value = '';

					$form = apply_filters( "wck_before_update_form_{$meta}_element_{$i}", $form, $element_id, $value );

					$form .= '<li class="row-'. esc_attr( Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details ) ) .'">';
	...
	function wck_output_entry_content( $meta, $id, $fields, $results, $element_id ){
		...
		if( !empty( $fields ) ){
			foreach( $fields as $field ){
				...
				if( !empty( $results[$element_id][Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details )] ) )
					$value = $results[$element_id][Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details )];
				else
					$value ='';
				...
				$list .= '<li class="row-'. esc_attr( Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details ) ) .'" data-type="'.$details['type'].'"><strong>'.$details['title'].': </strong>'.$display_value.' </li>' . "\r\n";
				...
				if ($meta == 'wck_cfc_fields') {
					if( !empty( $results[$element_id][Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details )] ) ){
						$field_title = $results[$element_id][Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $details['title'], $details )];
		...
		if( !empty( $fields ) ){
			foreach( $fields as $field ){
				if( !empty( $field['required'] ) && $field['required'] )
					$required_fields[Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $field['title'], $field )] = $field['title'];
			}
		}
                    ...
                    if( $this->args['meta_name'] == $meta_name ){
                        ...
                        if( !empty( $this->args['meta_array'] ) ){
                            foreach ($this->args['meta_array'] as $meta_field){
                                ...
                                $single_field_name = $this->args['meta_name'] .'_'. Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $meta_field['title'],$meta_field );
                                if (isset($_POST[$single_field_name])) {
                                    ...
                                    $meta_values[Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $meta_field['title'], $meta_field )] = $_POST[$single_field_name];
                                }
                                else
                                    $meta_values[Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $meta_field['title'], $meta_field )] = '';
                            }
                        }
                        ...
                        if( !empty( $errors ) ){
                            ...
                            if ($this->args['unserialize_fields']) {
                                if (!empty($this->args['meta_array'])) {
                                    foreach ($this->args['meta_array'] as $meta_field) {
                                        update_post_meta($post_id, $meta_name . '_' . Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $meta_field['title'], $meta_field ) . '_1', $_POST[$this->args['meta_name'] . '_' . Wordpress_Creation_Kit_PMSLE::wck_generate_slug( $meta_field['title'], $meta_field )]);
                                    }
                                }
                            }
                        }
