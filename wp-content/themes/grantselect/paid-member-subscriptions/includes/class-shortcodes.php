...
Class PMSWMG_Shortcodes {
    ...
    public static function member_account( $args ) {
        ...
        if (get_query_var( 'tab' ) != "manage-group"){
            if (count(pms_success()->get_messages( 'invite_members' )) == 0 ){
        ...
        $subscription_id = 0;
        $subscription = null;
        $own_subscription_id = 0;
        $own_subscription = null;

        if ( $args['show_tabs'] === true ) {
            // Output tabs
            $member          = pms_get_member( $user_id );
            $newsletter_sids   = [];
            $newsletter_plan_ids = array_keys(GS_NEWSLETTER_SUBSCRIPTION_PLANS);
            if ($member->subscriptions != NULL){
                foreach( $member->subscriptions as $subscript ){
                    ...
                    if (array_search($subscript['subscription_plan_id'], $newsletter_plan_ids) !== false){
                        $newsletter_sids[] =$subscript['id'];
                    }
                    if( $plan->type == 'group' ){
                        $own_subscription = pms_get_member_subscription($subscript['id']);
                        $own_subscription_id = $subscript['id'];
                    }
                    $subscription = pms_get_member_subscription($subscript['id']);
                    $subscription_id = $subscript['id'];
                }
            }
            if ($own_subscription_id != 0){
                $subscription = $own_subscription;
                $subscription_id = $own_subscription_id;
            }
                ...
                if (pms_gm_is_group_owner( $subscription->id )){
                    if (isset($_GET['edit_user'])){
                        $tabs = apply_filters( 'pms_member_account_tabs', array(
                            'profile'       => __( 'Account Settings', 'paid-member-subscriptions' ),
                            'manage-group'  => __( 'Manage Users', 'paid-member-subscriptions' ),
                            'email-alerts'  => __( 'Email Alerts', 'paid-member-subscriptions' ),
                            'subscriptions' => __( 'Subscriptions', 'paid-member-subscriptions' ),
                            'usage'         => __('Usage', 'paid-member-subscriptions'),
                            'gsm-help'          => __( 'Help', 'paid-member-subscriptions'),
                          ), $args );
                    }else{
                        $tabs = apply_filters( 'pms_member_account_tabs', array(
                            'profile'       => __( 'Account Settings', 'paid-member-subscriptions' ),
                    ...
                    if (empty($newsletter_sids)){
                        $gs_admin_user = wp_get_current_user();
                        if ( in_array( 'gs_child', (array) $gs_admin_user->roles ) ) {
                            $tabs = apply_filters( 'pms_member_account_tabs', array(
                                'profile'       => __( 'Account Settings', 'paid-member-subscriptions' ),
                                //'subscriptions' => __( 'Subscriptions', 'paid-member-subscriptions' ),
                                //'payments'      => __( 'Payments', 'paid-member-subscriptions' ),
            ...
            // if ( apply_filters( 'pms_member_account_logout_tab', true ) && !isset($_GET['edit_user']) )
                  ...
                  <ul>
                      <?php foreach( $tabs as $slug => $name ) : ?>
                            <?php 
                                $tab_url = "";
                                if ($slug == "usage"){
                                    //$tab_url = "/subscriber/gs-account-usage-download";
                                    //$tab_url = add_query_arg("download", "csv", $tab_url);
                                    $tab_url = pms_account_get_tab_url( $slug, $account_page );
                                }else{
                                    $tab_url = pms_account_get_tab_url( $slug, $account_page );
                                }
                                
                                if (isset($_GET['edit_user'])){
                                    $tab_url = add_query_arg("edit_user", $_GET['edit_user'], $tab_url);
                                }
                            ?>
                            <li class="pms-account-navigation-link pms-account-navigation-link--<?php echo $slug; ?>">
                                <a class="<?php echo ( $active_tab == $slug ? 'pms-account-navigation-link--active' : '' ); ?>" href="<?php echo esc_url( ( $slug != 'logout' ? $tab_url : wp_logout_url( apply_filters( 'pms_member_account_logout_url', "login" ) ) ) ); ?>"><?php echo $name; ?></a>
                            </li>
                      <?php endforeach; ?>
                  </ul>
                </nav>
        ...
        if ( $active_tab == 'subscriptions' ) {
            ...
            $iua = $wpdb->get_row($wpdb->prepare($sql, [$user_id]));
            if ($iua){
                echo '<div class="iua-warning err-msg wppb-warning alert">';
                echo __("There is a system issue preventing you from self-registering. Please contact your administrator for assistance.", "iua") . "<br>";
                if ($iua->ecode == ERROR_OVERSEAT){
                    //echo __("But your group's seat is not enough. Please contact the GrantSelect administrator.", "iua");
                }else if ($iua->ecode == ERROR_MULTIGROUP){
                    //echo __("But you have the multiple groups. To select a group, please contact the GrantSelect administrator.", "iua");
                }
                echo '</div>';
            }
            $member          = pms_get_member( $user_id );
            ...
            if ($subscription_id != 0){
                if (pms_gm_is_group_owner( $subscription_id )){
                    include get_stylesheet_directory() . '/pms-add-on-group-memberships/includes/views/view-edit-group-details.php';
                }
            } 
            if( defined('PROFILE_BUILDER') ){
                ...
                if( isset( $pms_general_settings['edit_profile_shortcode'] ) && $pms_general_settings['edit_profile_shortcode'] != '-1' ){
                    ...
                    if( $pms_general_settings['edit_profile_shortcode'] == 'wppb-default-edit-profile' )
                        ...
                        if ($subscription_id != 0 ){
                            if (!pms_gm_is_group_owner( $subscription_id )){
                                echo do_shortcode( '[wppb-edit-profile form_name="institutional-member-edit"]');    
                            }else{
                                //gs manager
                            ...
                            echo do_shortcode( '[wppb-edit-profile form_name="individual-account-edit"]');
                        }

                    }
                    if (isset($_GET['edit_user'])){
                        $gs_admin_user = wp_get_current_user();
                        if ( in_array( 'gs_admin', (array) $gs_admin_user->roles ) || in_array( 'administrator', (array) $gs_admin_user->roles ) ) {
                            echo do_shortcode('[wppb-edit-profile form_name="gs-admin-setting"]');
                            $sid = get_subscription_id($_GET['edit_user']);
                            echo do_shortcode('[gs-pms-update id="'.$sid.'"]');
                        }
                    }
        ...
        } else if ($active_tab == 'manage-group'){
            if (!isset($_GET['subscription_id'])) {
                if( !empty( $subscription->id ) && pms_gm_is_group_owner( $subscription->id )){
                    $_GET['subscription_id'] = $subscription_id;
                }
            }
            wp_enqueue_script( 'pms-gm-group-dashboard', plugins_url() . '/pms-add-on-group-memberships/assets/js/frontend-group-dashboard.js', array( 'jquery' ), PMS_GM_VERSION );
            wp_localize_script( 'pms-gm-group-dashboard', 'pms_gm', array(
                ...
                'remove_user_message'           => esc_html__( 'Are you sure you want to remove this user?', 'paid-member-subscriptions' ),
            ) );
            include get_stylesheet_directory() . '/pms-add-on-group-memberships/includes/class-group-memberships.php';
            include get_stylesheet_directory() . '/pms-add-on-group-memberships/includes/views/view-invite-members.php';
            include get_stylesheet_directory() . '/pms-add-on-group-memberships/includes/views/view-members-list.php';
            
            unset($_GET['subscription_id']);
        } else if ($active_tab == "email-alerts"){
            $subscription_id = 0;
            $subscription = null;
            $user_id = pms_get_current_user_id();
            $user = get_userdata($user_id);
            ...
            // Email Alert List
            include get_stylesheet_directory() . '/pms-add-on-group-memberships/includes/views/view-email-list.php';
            ...
            include get_stylesheet_directory() . '/pms-add-on-group-memberships/includes/views/view-usage.php';
        } else if ($active_tab == "gsm-help"){
            // Contact form for GS User
    ...
    public static function login_form( $atts ) {
        ...
        if( !is_user_logged_in() ) {
            ...
            if (isset($_SESSION['guest_user_id'])){
                $register_url = '/access/create-account/';
            }
